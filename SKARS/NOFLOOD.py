...
def check_flood(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        msg.reply_text(
            f"{ALKL}I can't restrict people here, give me permissions first! Until then, I'll disable anti-flood."
        )
    ...
    if match:
        ...
        try:
            ...
            update.effective_message.edit_text(
                f"{ALKL}Unmuted by {mention_html(user.id, html.escape(user.first_name))}.",
                parse_mode="HTML",
            )
        except:
            pass
    ...
    if len(args) >= 1:
        ...
        if val in ["off", "no", "0"]:
            ...
            if amount <= 0:
                ...
                send_message(
                    update.effective_message,
                    f"{ALKL}Antiflood must be either 0 (disabled) or number greater than 2!",
                )
                return ""
        ...
        message.reply_text(
            (
                f"{ALKL}Use `/setflood number` to enable anti-flood.\nOr use `/setflood off` to disable antiflood!."
            ),
            parse_mode="markdown",
        )
    return ""
...
def flood(update: Update, context: CallbackContext):
    ...
    if conn:
        ...
        if update.effective_message.chat.type == "private":
            send_message(
                update.effective_message,
                f"{ALKL}This command is meant to use in group not in PM",
            )
            return
        chat_id = update.effective_chat.id
        chat_name = update.effective_message.chat.title
...
def set_flood_mode(update: Update, context: CallbackContext):
    ...
    if args:
        if args[0].lower() == "ban":
            ...
            if len(args) == 1:
                teks = f"""{ALKL}It looks like you tried to set time value for antiflood but you didn't specified time; Try, `/setfloodmode tban <timevalue>`.
Examples of time value: 4m = 4 minutes, 3h = 3 hours, 6d = 6 days, 5w = 5 weeks."""
            ...
            if len(args) == 1:
                teks = (
                    update.effective_message,
                    f"""{ALKL}It looks like you tried to set time value for antiflood but you didn't specified time; Try, `/setfloodmode tmute <timevalue>`.
Examples of time value: 4m = 4 minutes, 3h = 3 hours, 6d = 6 days, 5w = 5 weeks.""",
                )
                send_message(update.effective_message, teks, parse_mode="markdown")
                return
...
FLOOD_BAN_WORK = MessageHandler(Filters.all & ~Filters.status_update & Filters.chat_type.groups, check_flood, run_async=True)
SET_FLOOD_WORK = CommandHandler("setflood", set_flood, filters=Filters.chat_type.groups, run_async=True)
SET_FLOOD_MODE_WORK = CommandHandler("setfloodmode", set_flood_mode, run_async=True)
FLOOD_QUERY_WORK = CallbackQueryHandler(flood_button, pattern=r"unmute_flooder", run_async=True)
FLOOD_WORK = CommandHandler("flood", flood, filters=Filters.chat_type.groups, run_async=True)

dispatcher.add_handler(FLOOD_BAN_WORK, FLOOD_GROUP)
dispatcher.add_handler(FLOOD_QUERY_WORK)
dispatcher.add_handler(SET_FLOOD_WORK)
dispatcher.add_handler(SET_FLOOD_MODE_WORK)
dispatcher.add_handler(FLOOD_WORK)

__handlers__ = [(FLOOD_BAN_WORK, FLOOD_GROUP),SET_FLOOD_WORK,FLOOD_WORK,SET_FLOOD_MODE_WORK,]
