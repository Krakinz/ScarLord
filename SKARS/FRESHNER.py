...
def set_blue_text_must_click(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if val in ("off", "no"):
            ...
            reply = f"{ALKL}Invalid argument.Accepted values are 'yes', 'on', 'no', 'off'"
...
def add_bluetext_ignore(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        reply = f"{ALKL}No command supplied to be ignored."
        message.reply_text(reply)
...
def remove_bluetext_ignore(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        reply = f"{ALKL}No command supplied to be unignored."
        message.reply_text(reply)
...
def bluetext_ignore_list(update: Update, context: CallbackContext):
    ...
    if global_ignored_list:
        text = f"{ALKL}The following commands are currently ignored globally from bluetext cleaning :\n"

        for x in global_ignored_list:
            text += f" - <code>{x}</code>\n"

    if local_ignore_list:
        text += f"{ALKL}The following commands are currently ignored locally from bluetext cleaning :\n"

        for x in local_ignore_list:
            text += f" - <code>{x}</code>\n"

    if text == "":
        text = f"{ALKL}No commands are currently ignored from bluetext cleaning."
