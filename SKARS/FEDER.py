...
def leave_fed(update: Update, context: CallbackContext):
    ...
    if getuser in "creator" or user.id in KLAW_LINGS:
...
def user_join_fed(update: Update, context: CallbackContext):
    ...
    if is_user_fed_owner(fed_id, user.id) or user.id in KLAW_LINGS:
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if fban:
        fed_name = info["fname"]
        # https://t.me/SƈαɾLσɾԃSupport/41606 // https://t.me/SƈαɾLσɾԃSupport/41619
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "SƈαɾLσɾԃ_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="SƈαɾLσɾԃ_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "SƈαɾLσɾԃ_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="SƈαɾLσɾԃ_fbanned_users.csv",
            ...
            with BytesIO() as file:
                ...
                for x in splitting:
                    ...
                    if str(import_userid) == str(OWNER_ID):
                        failed += 1
                        continue
                    if int(import_userid) in KLAW_LINGS:
                        failed += 1
                        continue
...
NEW_FED_WORK = CommandHandler("newfed", new_fed, run_async=True)
DEL_FED_WORK = CommandHandler("delfed", del_fed, run_async=True)
RENAME_FED = CommandHandler("renamefed", rename_fed, run_async=True)
JOIN_FED_WORK = CommandHandler("joinfed", join_fed, run_async=True)
LEAVE_FED_WORK = CommandHandler("leavefed", leave_fed, run_async=True)
PROMOTE_FED_WORK = CommandHandler("fpromote", user_join_fed, run_async=True)
DEMOTE_FED_WORK = CommandHandler("fdemote", user_demote_fed, run_async=True)
INFO_FED_WORK = CommandHandler("fedinfo", fed_info, run_async=True)
BAN_FED_WORK = DisableAbleCommandHandler(["fban", "sfban"], fed_ban, run_async=True)
UN_BAN_FED_WORK = CommandHandler("unfban", unfban, run_async=True)
FED_BROADCAST_WORK = CommandHandler("fbroadcast", fed_broadcast, run_async=True)
FED_SET_RULES_WORK = CommandHandler("setfrules", set_frules, run_async=True)
FED_GET_RULES_WORK = CommandHandler("frules", get_frules, run_async=True)
FED_CHAT_WORK = CommandHandler("chatfed", fed_chat, run_async=True)
FED_ADMIN_WORK = CommandHandler("fedadmins", fed_admin, run_async=True)
FED_USERBAN_WORK = CommandHandler("fbanlist", fed_ban_list, run_async=True)
FED_NOTIF_WORK = CommandHandler("fednotif", fed_notif, run_async=True)
FED_CHATLIST_WORK = CommandHandler("fedchats", fed_chats, run_async=True)
FED_IMPORTBAN_WORK = CommandHandler("importfbans", fed_import_bans, run_async=True)
...
DELETEBTN_FED_WORK = CallbackQueryHandler(del_fed_button, pattern=r"rmfed_", run_async=True)
FED_OWNER_HELP_WORK = CommandHandler("fedownerhelp", fed_owner_help, run_async=True)
FED_ADMIN_HELP_WORK = CommandHandler("fedadminhelp", fed_admin_help, run_async=True)
FED_USER_HELP_WORK = CommandHandler("feduserhelp", fed_user_help, run_async=True)

dispatcher.add_handler(NEW_FED_WORK)
dispatcher.add_handler(DEL_FED_WORK)
dispatcher.add_handler(RENAME_FED)
dispatcher.add_handler(JOIN_FED_WORK)
dispatcher.add_handler(LEAVE_FED_WORK)
dispatcher.add_handler(PROMOTE_FED_WORK)
dispatcher.add_handler(DEMOTE_FED_WORK)
dispatcher.add_handler(INFO_FED_WORK)
dispatcher.add_handler(BAN_FED_WORK)
dispatcher.add_handler(UN_BAN_FED_WORK)
dispatcher.add_handler(FED_BROADCAST_WORK)
dispatcher.add_handler(FED_SET_RULES_WORK)
dispatcher.add_handler(FED_GET_RULES_WORK)
dispatcher.add_handler(FED_CHAT_WORK)
dispatcher.add_handler(FED_ADMIN_WORK)
dispatcher.add_handler(FED_USERBAN_WORK)
dispatcher.add_handler(FED_NOTIF_WORK)
dispatcher.add_handler(FED_CHATLIST_WORK)
# dispatcher.add_handler(FED_IMPORTBAN_WORK)
...
dispatcher.add_handler(DELETEBTN_FED_WORK)
dispatcher.add_handler(FED_OWNER_HELP_WORK)
dispatcher.add_handler(FED_ADMIN_HELP_WORK)
dispatcher.add_handler(FED_USER_HELP_WORK)
