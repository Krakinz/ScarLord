...
import SKARSHOTS.welcome_sql as sql
...
def send(update, message, keyboard, backup_message):
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            ...
            msg = update.effective_message.reply_text(
                markdown_parser(
                    backup_message + f"{ALKL}Note: An error occured when sending the "
    ...
    for new_mem in new_members:
            ...
            if should_mute:
                ...
                if welc_mutes == "strong":
                    ...
                    message = msg.reply_text(
                        f"{ALKL}{new_join_mem}, click the button below to prove you're human.\nYou have 120 seconds.",
        ...
        if user.id == new_mem.id:
            welcome_log = (
                f"{html.escape(chat.title)}\n"
                f"#USER_JOINED\n"
                f"{ALKL}<b>User</b>: {mention_html(user.id, user.first_name)}\n"
            ...
            welcome_log = (
                f"{html.escape(chat.title)}\n"
                f"#BOT_ADDED\n"
                f"{ALKL}<b>Bot</b>: {mention_html(new_mem.id, new_mem.first_name)}\n"
            ...
            welcome_log = (
                f"{html.escape(chat.title)}\n"
                f"#USER_ADDED\n"
                f"{ALKL}<b>User</b>: {mention_html(new_mem.id, new_mem.first_name)}\n"
                f"<b>ID</b>: <code>{new_mem.id}</code>"
            )
        return welcome_log
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            # Give the devs a special goodbye
                ...
                update.effective_message.reply_text(
                    f"{ALKL}See you later at the Hero's Association!",
                    reply_to_message_id=reply,
                )
                return
...
def welcome(update: Update, context: CallbackContext):
    ...
    if not args or args[0].lower() == "noformat":
        ...
        update.effective_message.reply_text(
            f"{ALKL}This chat has it's welcome setting set to: `{pref}`.\n"
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            update.effective_message.reply_text(
                f"{ALKL}Okay! I'll greet members when they join.",
            )

        elif args[0].lower() in ("off", "no"):
            sql.set_welc_preference(str(chat.id), False)
            update.effective_message.reply_text(
                f"{ALKL}I'll go loaf around and not welcome anyone then.",
            )

        else:
            update.effective_message.reply_text(
                f"{ALKL}I understand 'on/yes' or 'off/no' only!",
            )
...
def goodbye(update: Update, context: CallbackContext):
    ...
    if not args or args[0] == "noformat":
        ...
        update.effective_message.reply_text(
            f"{ALKL}This chat has it's goodbye setting set to: `{pref}`.\n"
...
def welcomemute(update: Update, context: CallbackContext) -> str:
    ...
    if len(args) >= 1:
        if args[0].lower() in ("off", "no"):
            ...
            msg.reply_text(
                f"{ALKL}I will restrict users' permission to send media for 24 hours.",
            )
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"#WELCOME_MUTE\n"
            ...
            msg.reply_text(
                f"{ALKL}I will now mute people when they join until they prove they're not a bot.\nThey will have 120seconds before they get kicked.",
            )
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"#WELCOME_MUTE\n"
...
def cleanservice(update: Update, context: CallbackContext) -> str:
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 1:
            ...
            update.effective_message.reply_text(
                f"{ALKL}Usage is <code>on</code>/<code>yes</code> or <code>off</code>/<code>no</code>",
                parse_mode=ParseMode.HTML,
            )
    else:
...
def user_button(update: Update, context: CallbackContext):
    ...
    if join_user == user.id:
        ...
        query.answer(text=f"{ALKL}Yeet! You're a human, unmuted!")
        bot.restrict_chat_member(
            chat.id,
            user.id,
            permissions=ChatPermissions(
        ...
        query.answer(text=f"{ALKL}You're not allowed to do this!")
...
WELC_HELP_TXT = (
    ...
    "⚔️ •{last}-\n this represents the user's *last* name. Defaults to *first name* if user has no "
    "last name.\n"
    "⚔️ •{fullname}-\n this represents the user's *full* name. Defaults to *first name* if user has no "
    "last name.\n"
    "⚔️ •{username}-\n this represents the user's *username*. Defaults to a *mention* of the user's "
    "first name if has no username.\n"
    "⚔️ •{mention}-\n this simply *mentions* a user - tagging them with their first name.\n"
    "⚔️ •{id}-\n this represents the user's *id*\n"
    "⚔️ •{count}-\n this represents the user's *member number*.\n"
    "⚔️ •{chatname}-\n this represents the *current chat name*.\n"
...
WELC_MUTE_HELP_TXT = (
    ...
    "⚔️ •/welcomemute strong-\n mutes new members till they tap on a button thereby verifying they're human.\n"
    "⚔️ •/welcomemute off-\n turns off welcomemute.\n"
    "*Note*-\n Strong mode kicks a user from the chat if they dont verify in 120seconds. They can always rejoin though"
)
...
⚔️ •/setwelcome <sometext>-\n set a custom welcome message. If used replying to media, uses that media.
⚔️ •/setgoodbye <sometext>-\n set a custom goodbye message. If used replying to media, uses that media.
⚔️ •/resetwelcome-\n reset to the default welcome message.
⚔️ •/resetgoodbye-\n reset to the default goodbye message.
⚔️ •/cleanwelcome <on/off>-\n On new member, try to delete the previous welcome message to avoid spamming the chat.
⚔️ •/welcomemutehelp-\n gives information about welcome mutes.
⚔️ •/cleanservice <on/off-\n deletes telegrams welcome/left service messages.
 *Example:*
user joined chat, user left chat.
*Welcome markdown:*
⚔️ •/welcomehelp-\n view more formatting information for custom welcome/goodbye messages.
"""
NEW_MEM_WORK = MessageHandler(Filters.status_update.new_chat_members, new_member, run_async=True)
LEFT_MEM_WORK = MessageHandler(Filters.status_update.left_chat_member, left_member, run_async=True)
WELC_PREF_WORK = CommandHandler("welcome", welcome, filters=Filters.chat_type.groups, run_async=True)
GOODBYE_PREF_WORK = CommandHandler("goodbye", goodbye, filters=Filters.chat_type.groups, run_async=True)
...
WELCOMEMUTE_WORK = CommandHandler("welcomemute", welcomemute, filters=Filters.chat_type.groups, run_async=True)
CLEAN_SERVICE_WORK = CommandHandler(
...
BUTTON_VERIFY_WORK = CallbackQueryHandler(user_button, pattern=r"user_join_", run_async=True)

dispatcher.add_handler(NEW_MEM_WORK)
dispatcher.add_handler(LEFT_MEM_WORK)
dispatcher.add_handler(WELC_PREF_WORK)
dispatcher.add_handler(GOODBYE_PREF_WORK)
...
dispatcher.add_handler(WELCOMEMUTE_WORK)
dispatcher.add_handler(CLEAN_SERVICE_WORK)
dispatcher.add_handler(BUTTON_VERIFY_WORK)
dispatcher.add_handler(WELCOME_MUTE_HELP)


__command_list__ = []
__handlers__ = [NEW_MEM_WORK,LEFT_MEM_WORK,WELC_PREF_WORK,GOODBYE_PREF_WORK,SET_WELCOME,SET_GOODBYE,RESET_WELCOME,
RESET_GOODBYE,CLEAN_WELCOME,WELCOME_HELP,WELCOMEMUTE_WORK,CLEAN_SERVICE_WORK,BUTTON_VERIFY_WORK,WELCOME_MUTE_HELP]
